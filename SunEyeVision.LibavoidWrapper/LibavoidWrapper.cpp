...
namespace SunEyeVision {
    ...
    namespace LibavoidWrapper {
            ...
            List<ManagedRect>^ obstacles)
        {
            ...
            // DEBUG: Log input parameters to file
            String^ logPath = "C:\\temp\\libavoid_debug.log";
            System::IO::File::AppendAllText(logPath,
                String::Format("[CalculateOrthogonalPathInternal] Source:({0:F1},{1:F1}), Target:({2:F1},{3:F1})\n",
                source.X, source.Y, target.X, target.Y));

            // Add source point as-is (no adjustment)
            System::IO::File::AppendAllText(logPath,
                String::Format("[CalculateOrthogonalPathInternal] Adding source point: ({0:F1},{1:F1})\n", source.X, source.Y));
            path->Add(source);
