...
# -*- coding: utf-8 -*-
"""
SunEyeVision 编码问题修复脚本 v7
修复UTF-8中文被截断的问题 - 同时处理Unicode替换字符和普通问号
"""
...
CONTEXT_FIXES = [
    ...
    (r'<param name="key">配设置\?</param>', '<param name="key">配置键</param>'),
    (r'<param name="value">配设置\?</param>', '<param name="value">配置值</param>'),
    (r'<returns>配设置\?</returns>', '<returns>配置值</returns>'),
    (r'<returns>所有配设置\?</returns>', '<returns>所有配置</returns>'),
]
...
def fix_content(content: str) -> Tuple[str, int]:
    ...
    # 检查是否有问题字符（Unicode替换字符或中文后的问号）
    replacement_char = '\ufffd'
    
    if replacement_char not in content and not re.search(r'[\u4e00-\u9fff]\?', content):
        return content, 0
    ...
    # 单字符修复 - 中文 + 替换字符 或 中文 + 普通问号
    for wrong, correct in FIXES.items():
        # 处理Unicode替换字符
        pattern1 = wrong.replace('?', replacement_char)
        if pattern1 in content:
            count = content.count(pattern1)
            content = content.replace(pattern1, correct)
            fix_count += count
...
def main():
    print("=" * 70)
    print("SunEyeVision 编码问题修复脚本 v7")
    print("=" * 70)
