    ...
    public static class LibraryValidator
    {
        ...
        public static void InspectDiagramControlProperties(object diagramControl)
        {
            try
            {
                ...
                System.Diagnostics.Debug.WriteLine("========== DiagramControl 属性 ==========");
                System.Diagnostics.Debug.WriteLine($"类型: {diagramType.FullName}");
                System.Diagnostics.Debug.WriteLine($"属性总数: {properties.Length}");

                System.Diagnostics.Debug.WriteLine("\n所有属性:");
                foreach (var prop in properties)
                {
                    var value = prop.GetValue(diagramControl);
                    var valueStr = value != null ? $"{value.GetType().Name}" : "null";
                    System.Diagnostics.Debug.WriteLine($"  - {prop.Name}: {prop.PropertyType.Name} = {valueStr}");
                }
                ...
                if (sourceItemsContainerProp != null)
                {
                    ...
                    if (container != null)
                    {
                        System.Diagnostics.Debug.WriteLine($"\n========== SourceItemsContainer 详情 ==========");
                        System.Diagnostics.Debug.WriteLine($"类型: {container.GetType().FullName}");

                        var containerProps = container.GetType().GetProperties(BindingFlags.Public | BindingFlags.Instance);
                        System.Diagnostics.Debug.WriteLine($"属性总数: {containerProps.Length}");
                        ...
                        foreach (var method in methods.Where(m => !m.IsSpecialName))
                        {
                            ...
                            System.Diagnostics.Debug.WriteLine($"  - {method.Name}({parameters}): {method.ReturnType.Name}");
                        }

                        System.Diagnostics.Debug.WriteLine("\n========== SourceItemsContainer 检查完成 ==========");
                    }
                    else
                    {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"❌ 检查属性失败: {ex.Message}");
