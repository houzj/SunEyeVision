...
#### 目标
- 将2300行的WorkflowCanvasControl拆分为多个职责单一的类
- 提高代码可维护性和可测试性
...
|--------|---------|---------|------|--------|
| arch-1 | 创建WorkflowCanvasService目录结构，拆分节点拖拽处理逻辑到NodeDragHandler类 | 8h | - | 高 |
| arch-2 | 创建ConnectionDragHandler类，提取连接创建和拖拽逻辑 | 12h | arch-1 | 高 |
| arch-3 | 创建BoxSelectionHandler类，提取框选功能 | 6h | arch-1 | 高 |
| arch-4 | 创建PortInteractionHandler类，提取端口交互逻辑 | 8h | arch-1 | 高 |
| code-1 | 创建CanvasConfig配置类，消除魔法数字 | 4h | - | 高 |
| code-2 | 提取GetPortPosition等重复代码为辅助方法 | 4h | - | 高 |

#### Week 2: 重构集成

| 任务ID | 任务描述 | 预计工时 | 依赖 | 优先级 |
|--------|---------|---------|------|--------|
| arch-5 | 重构WorkflowCanvasControl主类，使用新的处理器类 | 16h | arch-1~arch-4 | 高 |
| test-1 | 为NodeDragHandler编写单元测试 | 6h | arch-1 | 高 |
| test-2 | 为ConnectionDragHandler编写单元测试 | 8h | arch-2 | 高 |
...
| 任务ID | 任务描述 | 预计工时 | 依赖 | 优先级 |
|--------|---------|---------|------|--------|
| cmd-1 | 设计并实现ICanvasCommand接口 | 4h | - | 高 |
| cmd-2 | 实现MoveNodeCommand、CreateConnectionCommand等具体命令 | 12h | cmd-1 | 高 |
| cmd-3 | 实现UndoRedoManager管理器 | 8h | cmd-2 | 高 |
| cmd-4 | 在WorkflowCanvasControl中集成撤销重做功能 | 8h | cmd-3, arch-5 | 高 |
| test-3 | 为命令模式编写单元测试 | 8h | cmd-2 | 高 |
...
| 任务ID | 任务描述 | 预计工时 | 依赖 | 优先级 |
|--------|---------|---------|------|--------|
| perf-1 | 实现节点四叉树索引，优化节点查找 | 12h | - | 高 |
| perf-2 | 优化HitTest调用频率，添加时间间隔控制 | 6h | perf-1 | 高 |
| perf-3 | 使用条件编译优化调试日志输出 | 4h | - | 中 |
| perf-4 | 实现日志级别控制（Error、Warning、Info、Debug） | 6h | perf-3 | 中 |
...
| 任务ID | 任务描述 | 预计工时 | 依赖 | 优先级 |
|--------|---------|---------|------|--------|
| feature-1 | 实现键盘快捷键系统（Delete、Ctrl+Z、Ctrl+Y等） | 8h | cmd-4 | 中 |
| feature-2 | 实现ZoomPanBehavior，支持画布缩放和平移 | 12h | - | 中 |
| feature-3 | 实现SnapHelper，支持节点对齐和网格吸附 | 8h | - | 中 |
| code-3 | 在关键操作中添加异常处理和用户友好提示 | 6h | - | 中 |

#### Week 6: 用户体验

| 任务ID | 任务描述 | 预计工时 | 依赖 | 优先级 |
|--------|---------|---------|------|--------|
| feature-4 | 添加拖拽预览效果 | 6h | - | 中 |
| feature-5 | 添加连接验证提示 | 6h | - | 中 |
| test-4 | 为关键功能编写集成测试 | 12h | feature-1~feature-5 | 中 |
...
| 任务ID | 任务描述 | 预计工时 | 依赖 | 优先级 |
|--------|---------|---------|------|--------|
| ui-1 | 添加节点出现动画效果 | 8h | - | 低 |
| ui-2 | 优化连接线渲染性能 | 10h | - | 低 |
...
| 指标 | 当前值 | 目标值 | 测量方法 |
|------|--------|--------|---------|
| 代码行数 | 2300行 | 1400行 | 统计代码行数 |
| 单元测试覆盖率 | 0% | >85% | 测试覆盖率工具 |
| 性能（100节点） | 卡顿 | <50ms响应 | 性能测试工具 |
| Bug率 | 基准 | 降低60% | Bug跟踪系统 |
...
#### 立即开始（第1周）
- ✅ arch-1：创建Interaction目录结构
- ✅ code-1：创建CanvasConfig配置类
- ✅ code-2：提取辅助方法

#### 第1周完成
- ✅ arch-1~arch-4：创建所有处理器类
- ✅ code-1~code-2：配置类和辅助方法
