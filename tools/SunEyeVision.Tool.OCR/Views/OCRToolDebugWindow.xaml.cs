...
namespace SunEyeVision.Tool.OCR.Views
{
    ...
    public partial class OCRToolDebugWindow : BaseToolDebugWindow
    {
        ...
        private ParamComboBox? _languageCombo;
        private ParamTextBox? _dataPathTextBox;
        private ParamTextBox? _charWhitelistTextBox;
        private ParamComboBox? _psmModeCombo;

        private OCRParameters _parameters = null!;
        ...
        {
            Tool = toolPlugin;
            NodeName = toolMetadata?.DisplayName ?? "OCR识别";
        ...
        private void SetupBindingsAndEvents()
        {
            ...
            {
                _languageCombo.ItemsSource = new[] { "chi_sim", "chi_tra", "eng", "chi_sim+eng", "jpn", "kor", "fra", "deu", "spa" };
                var binding = new Binding("Language")
                {
                    Source = _parameters,
                    Mode = BindingMode.TwoWay
                };
                _languageCombo.SetBinding(ParamComboBox.SelectedItemProperty, binding);
            }
        }
        ...
        public void SetCurrentNode(object node)
        {
            ...
            if (parameters is OCRParameters ocrParams)
            {
                _parameters = ocrParams;
                SetupBindingsAndEvents();
                PluginLogger.Success($"参数引用已设置: Language={ocrParams.Language}", "OCRTool");
            }
        }
        ...
        protected override void OnExecuteRequested()
        {
            ...
            {
                PluginLogger.Warning("无法获取图像数据", "OCRTool");
                return;
            }
        ...
        {
            if (Tool is not OCRTool ocrTool)
            {
                PluginLogger.Error("工具实例无效", "OCRTool");
                return;
            }
            ...
            try
            {
                var runParams = (OCRParameters)_parameters.Clone();
                OCRResults result = ocrTool.Run(inputImage, runParams);
                ...
                Dispatcher.Invoke(() =>
                {
                    if (result.IsSuccess)
                    {
                        PluginLogger.Success($"识别成功 - 文本: {result.Text}", "OCRTool");
                    }
                    else
                    {
                        PluginLogger.Error($"识别失败 - {result.ErrorMessage}", "OCRTool");
                    }
                });
            }
            catch (Exception ex)
            {
                stopwatch.Stop();
                PluginLogger.Error($"执行异常: {ex.Message}", "OCRTool");
            }
        }
