...
namespace SunEyeVision.UI.Controls.Rendering
{
    ...
    public class GPUCache
    {
        ...
        {
            var cacheFile = Path.Combine(_cachePath, $"{cacheKey}.cache");
        ...
        private void SaveToDiskCache(string cacheKey, BitmapImage bitmap)
        {
            ...
            try
            {
                ...
                Debug.WriteLine($"[GPUCache] ✓ 已保存到磁盘缓存: {Path.GetFileName(cacheFile)}");
            ...
            try
            {
                ...
                while (totalSize > targetSize && files.Count > 0)
                {
                    ...
                    try
                    {
                        ...
                        lock (_lockObj)
                        {
                            _diskCacheKeys.Remove(Path.GetFileNameWithoutExtension(file.Name));
                        }
                    }
                    catch { }
                    files.RemoveAt(0);
                ...
                if (removedCount > 0)
                {
                    Debug.WriteLine($"[GPUCache] ✓ 清理磁盘缓存 - 删除了 {removedCount} 个文件");
                }
            }
            catch (Exception ex)
            {
