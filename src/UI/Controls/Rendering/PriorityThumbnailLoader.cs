...
namespace SunEyeVision.UI.Controls.Rendering
{
    ...
    public enum LoadPriority
    {
        ...
        Low = 3,
        /// <summary>空闲优先级 - 仅在空闲时加载，可随时取消</summary>
        Idle = 4
    }
    ...
    /// <summary>
    /// 缓冲区域配置
    /// ★ 单一数据源架构：所有值基于实际可视数量动态派生
    /// </summary>
    public class BufferZoneConfig
    {
        private int _actualVisibleSize = 10; // 由外部设置
        ...
        /// <summary>预加载区域大小（单侧，派生：可视区域的50%）</summary>
        ...
        /// <summary>静止时后备区域激活延迟(ms)</summary>
    ...
    public class PriorityThumbnailLoader : IDisposable
    {
        ...
        private const int MIN_CONCURRENCY = 4; // ★ 优化：提高最小并发度，避免过低
        private const int MAX_CONCURRENCY = 8;
        private const int STARTUP_GRACE_PERIOD_MS = 3000; // ★ 新增：首屏并发调整延迟3秒
        ...
        public PriorityThumbnailLoader()
        {
            ...
            Debug.WriteLine($"[PriorityLoader] ✓ 线程池分离 - 高优先级:{HIGH_PRIORITY_THREADS}, 普通:{normalThreads}, 总计:{totalThreads}");
        }
        ...
        public void CancelAndReset()
        {
            ...
            Debug.WriteLine($"[诊断] CancelAndReset详情: Cancel={cancelMs}ms, Sleep={sleepMs}ms, ClearQueue={clearQueueMs}ms, ClearUiQueue={clearUiQueueMs}ms");
            Debug.WriteLine("[PriorityLoader] ✓ 已取消旧任务并重置状态");
        }
            ...
            Action<int>? onFirstImageLoaded = null)
        {
            ...
            _visibleAreaCount = loadCount;  // ★ 统一：使用实际可视数量
            _loadedInVisibleArea = 0;
            ...
            if (fileNames.Length > 0 && imageCollection.Count > 0)
            {
                ...
                Debug.WriteLine($"[PriorityLoader] ▶ Critical同步加载开始 | file={firstFileName}");
                ...
                if (thumbnail != null)
                {
                    // ★ 关键优化：使用Normal优先级，避免被后台任务饿死
                    // Loaded优先级太低，在大量后台任务时会延迟1-2秒
                    stepSw.Restart();
            ...
            Debug.WriteLine($"  实际可视数量: {visibleCount}张 (来自GetVisibleRange)");
            Debug.WriteLine($"  可视区域: [0同步] + [1-{visibleCount - 1}] ({visibleCount}张)");
            Debug.WriteLine($"  缓冲区域(Medium): [{visibleCount}-{visibleCount + bufferSize - 1}] ({bufferSize}张)");
            Debug.WriteLine($"  预测区域(Low): [{visibleCount + bufferSize}-{totalEnqueueCount - 1}] ({prefetchSize}张)");
            Debug.WriteLine($"  ★ 入队总数: {totalEnqueueCount - 1} / {fileNames.Length} (首张已同步加载)");
        ...
        public void UpdateVisibleRange(int firstVisible, int lastVisible, int totalCount)
        {
            ...
            int visibleCount = actualVisibleCount;  // 统一使用实际值
            int needLoadCount = 0;
            ...
            {
                _visibleAreaLoadStartTime = DateTime.Now;
                _visibleAreaCount = visibleCount;  // ★ 统一：使用实际可视数量
                _loadedInVisibleArea = 0;
            }
        ...
        /// <summary>
        /// ★ P1优化：根据内存压力动态调整并发度
        /// 在 ImagePreviewControl 中连接内存监控事件时调用
        /// </summary>
        public void SetMemoryPressure(MemoryPressureMonitor.PressureLevel level)
        {
            ...
            Debug.WriteLine($"[PriorityLoader] 内存压力:{level} → 并发度:{_dynamicConcurrency} 低质量:{_useLowQuality}");
        }
        ...
        private void CancelLowPriorityTasks()
        {
            lock (_queueLock)
            {
                ...
                {
                    // 只保留 Critical、High、Medium 优先级任务
                ...
                if (toRemove.Count > 0)
                {
                    Debug.WriteLine($"[PriorityLoader] ✓ 取消低优先级任务: {toRemove.Count}个");
                }
            }
        }
        ...
        private LoadPriority GetPriorityForBufferZone(BufferZone zone, int index, int firstVisible)
        {
            return zone switch
            {
                ...
                BufferZone.Reserve => LoadPriority.Idle, // ★ P3空闲优先级
                _ => LoadPriority.Low
            };
        }
        ...
        private void UpdateScrollState(int firstVisible, int lastVisible, int totalCount)
        {
            ...
            if (_lastFirstVisible >= 0)
            {
                ...
                if (_currentScrollType != previousScrollType)
                {
                    ...
                    {
                        Debug.WriteLine($"[DynamicRange] 滚动类型变化: {previousScrollType} → {_currentScrollType} (速度:{_scrollVelocity:F1}项/秒)");
                    }
                    else if (_currentScrollType == ScrollType.Stopped && previousScrollType != ScrollType.Stopped)
                    {
                        Debug.WriteLine($"[DynamicRange] 滚动停止 - 开始加载可视区域");
                    }
                }
                ...
                {
                    Debug.WriteLine($"[DynamicRange] 快速滚动开始 - 速度:{_scrollVelocity:F1}项/秒");
                }
        ...
        private void LoadReserveZone(int totalCount)
        {
            ...
            Debug.WriteLine($"[BufferZone] 后备区域激活 - 左:[{_reserveZoneLeft.start}-{_reserveZoneLeft.end}] 右:[{_reserveZoneRight.start}-{_reserveZoneRight.end}]");
        ...
        private async Task LoadSingleThumbnailAsync(LoadTask task, CancellationToken cancellationToken)
        {
            ...
            // ★ P0优化 - 阶段1：开始前检查 - 低优先级任务在高负载时放弃
            if (task.Priority >= LoadPriority.Low && _useLowQuality)
            {
                Debug.WriteLine($"[PriorityLoader] ⚠ 放弃Low任务 index={task.Index}（低质量模式）");
                return;
            }
            ...
            lock (_queueLock)
            {
                ...
                // ★ 关键修改：使用 _decodingIndices 标记正在解码，而不是 _loadedIndices
        ...
        private class UIUpdateRequest
        {
            ...
            public string FilePath { get; set; } = string.Empty;
            public bool DecodeSuccess { get; set; }  // ★ 新增：标记解码是否成功
        }
