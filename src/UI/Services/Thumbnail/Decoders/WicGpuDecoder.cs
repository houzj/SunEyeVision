            ...
            try
            {
                ...
                try
                {
                    ...
                    if (verboseLog)
                    {
                        ...
                        Debug.WriteLine($"[诊断] WicGpuDecoder详情: 等待={waitSw.Elapsed.TotalMilliseconds:F0}ms 读取={readSw.Elapsed.TotalMilliseconds:F0}ms 分辨率={infoSw.Elapsed.TotalMilliseconds:F0}ms 解码={decodeSw.Elapsed.TotalMilliseconds:F0}ms Freeze={freezeSw.Elapsed.TotalMilliseconds:F0}ms 总计={totalSw.Elapsed.TotalMilliseconds:F0}ms | {diagnosticInfo} | file={System.IO.Path.GetFileName(filePath)}");
