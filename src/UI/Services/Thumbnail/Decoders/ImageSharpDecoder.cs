...
namespace SunEyeVision.UI.Services.Thumbnail.Decoders
{
    ...
    public class ImageSharpDecoder : IThumbnailDecoder
    {
        ...
        public bool Initialize()
        {
            ...
            {
                Debug.WriteLine($"[ImageSharpDecoder] �?初始化失�? {ex.Message}");
                return false;
            }
        }
        ...
        public BitmapImage? DecodeThumbnail(string filePath, int size, byte[]? prefetchedData = null, bool verboseLog = false, bool isHighPriority = false)
        {
            ...
            try
            {
                ...
                {
                    Debug.WriteLine($"[ImageSharpDecoder] �?文件不存�? {filePath}");
                    return null;
                }
                ...
                if (!gotSlot)
                {
                    if (isHighPriority)
                    {
                        Debug.WriteLine($"[ImageSharpDecoder] �?等待超时({waitTimeout}ms)，跳过高优先级任�?| file={System.IO.Path.GetFileName(filePath)}");
                    }
                    else
                    {
        ...
        public void Dispose()
        {
            ...
            Debug.WriteLine("[ImageSharpDecoder] 资源已释�?);
        }
    }
}
